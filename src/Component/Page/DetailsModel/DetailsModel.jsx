...
const DetailsModel = () => {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setModel((prev) => ({
          ...prev,
          purchased: data.purchasedCount || prev.purchased + 1,
        }));
      } else {
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const data = await res.json();
        if (res.ok) {
          Swal.fire("Deleted!", "Model deleted successfully.", "success");
          navigate("/models");
        } else {
  ...
  if (!model) return <p className="text-center text-red-400 mt-10 text-lg">Model not found.</p>;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-10 px-6">
      <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 backdrop-blur-lg border border-white/20 rounded-2xl shadow-xl p-8 max-w-3xl text-white">
        ...
        <h2 className="text-4xl font-bold mb-3 text-gray-100 drop-shadow">{model.name}</h2>
        <p className="text-gray-100 mb-2">
          <span className="font-semibold text-indigo-200">Framework:</span> {model.framework || "N/A"}
        </p>
        <p className="text-gray-100 mb-2">
          <span className="font-semibold text-indigo-200">Use Case:</span> {model.useCase || "N/A"}
        </p>
        <p className="text-gray-100 mb-2">
          <span className="font-semibold text-indigo-200">Dataset:</span> {model.dataset || "N/A"}
        </p>
        <p className="text-gray-200 mt-4">{model.description || "No description available."}</p>
        <p className="mt-6 text-sm text-gray-300">Created by: <span className="text-indigo-200">{model.createdBy || "Unknown"}</span></p>
        <p className="text-sm text-gray-300">Purchased {model.purchased || 0} times</p>
